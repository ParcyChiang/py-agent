## py-agent搭建Log
## v2.1
1. feat: 地图页面数量输入框支持前端实时调整（localStorage持久化）
2. fix: 修复选择"全部城市"时仍按城市过滤的问题
...
### v1.15
1. feat: 新增AI对话历史功能，左侧边栏存储历史对话记录
2. feat: 对话历史支持按页面筛选（代码生成/运营报告/物流对比）
3. feat: 对话历史支持关键词搜索
4. feat: 点击历史记录跳转到对应页面并恢复数据
5. feat: 新增 chat_history 数据库表存储对话记录
6. feat: 新增 `/api/chat_history/*` 系列API（创建/查询/删除/搜索）
